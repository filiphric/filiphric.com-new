<template>
  <div>
    <div v-if="query.length" class="baseline grid grid-cols-1 gap-7 md:grid-cols-2 xl:grid-cols-3">
...
const { data } = await useAsyncData('home', () => queryContent<MaybeComputedRef<any>>('/').where({ published: true }).sort({ date: -1 }).find())
